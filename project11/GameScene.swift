...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    override func didMove(to view: SKView) {
        ...
        ballsNumberLabel.text = "balls: 0"
        ballsNumberLabel.position = CGPoint(x: 250, y: 700)
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if objects.contains(editLabel) {
            ...
            if editingMode {
                ...
                ball.position = CGPoint(x: location.x, y: self.frame.maxY - 8)
                ball.name = "ball"
                addChild(ball)
                ballsNumber -= 1
            }
    ...
    func getRandomBall() -> String {
        let ballNames = ["ballRed", "ballYellow", "ballPurple", "ballGrey", "ballGreen", "ballCyan", "ballBlue"]
        let random = Int.random(in: 0..<ballNames.count)
        return ballNames[random]
    }
